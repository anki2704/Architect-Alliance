...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex flex-col bg-[var(--bg-main)] text-[var(--text-primary)]">
      {/* Top bar */}
      <header className="h-14 sm:h-16 shrink-0 border-b border-[var(--text-primary)]/10 bg-[var(--bg-surface)] flex items-center justify-between px-3 sm:px-6 gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          <div className="relative">
            <button
              ...
              className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[var(--bg-card)]/5 border border-[var(--text-primary)]/10 flex items-center justify-center hover:bg-[var(--bg-card)]/10 cursor-pointer"
              title="Notifications"
            >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-full bg-[var(--bg-card)]/5 border border-[var(--text-primary)]/15 text-xs font-semibold hover:bg-[var(--bg-card)]/10 cursor-pointer max-w-[140px] sm:max-w-none"
            >
              <span className="max-w-[72px] sm:max-w-[100px] truncate">{currentUser.name}</span>
              <span className="hidden xs:inline sm:inline px-1.5 sm:px-2 py-0.5 rounded-full bg-[var(--bg-card)] text-[var(--text-on-accent)] text-[10px] font-bold uppercase shrink-0">
                {role}
              </span>
            ...
            {roleMenuOpen && (
              <div
                className="absolute right-0 mt-2 w-48 rounded-xl bg-[var(--bg-card)] border border-[var(--text-primary)]/15 shadow-xl py-1 z-50"
                onClick={(e) => e.stopPropagation()}
              >
                <button
                  type="button"
                  onPointerDown={(e) => {
                    // Use pointerdown so mobile doesn't miss the click when menu closes
                    e.preventDefault();
                    e.stopPropagation();
                    setRoleMenuOpen(false);
                    onClose();
                  }}
                  className="w-full text-left px-4 py-2.5 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-card)]/10 cursor-pointer"
                >
                  Back to website
                </button>
              </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-full bg-red-500/10 border border-red-500/30 text-red-300 text-xs font-semibold hover:bg-red-500/20 cursor-pointer shrink-0"
            title="Logout"
          >
